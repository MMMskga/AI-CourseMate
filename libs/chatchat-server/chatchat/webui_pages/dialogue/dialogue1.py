...
def dialogue_page(
    ...
    with bottom():
        # 重新规划四列比例：[1]配置按钮，[1.5]微信麦克风，[14]输入框，[1]清空按钮
        cols = st.columns([1, 1.5, 14, 1])
        
        # 1. 齿轮配置按钮 (第一列)
        if cols[0].button(":gear:", help="模型配置", use_container_width=True):
            widget_keys = ["platform", "llm_model", "temperature", "system_message"]
            chat_box.context_to_session(include=widget_keys)
            llm_model_setting()
            
        # 2. 垃圾桶清空按钮 (第四列)
        if cols[-1].button(":wastebasket:", help="清空对话", use_container_width=True):
            chat_box.reset_history()
            rerun()
        ...
        with cols[1]:
            audio_bytes = audio_recorder(
                text="", 
                recording_color="#ef4444",  # 录音时闪烁红色
                neutral_color="#94a3b8",    # 平时呈现高级灰
                icon_size="1.8x",
                key="chat_mic"
            )
            ...
            if audio_bytes:
                with st.spinner("🧠..."):
                    try:
                        with open("temp_chat_audio.wav", "wb") as f:
                            f.write(audio_bytes)
                        
                        # 建立语音客户端
                        client_audio = openai.Client(base_url=f"{api_address()}/v1", api_key="NONE")
                        with open("temp_chat_audio.wav", "rb") as audio_file:
                            transcript = client_audio.audio.transcriptions.create(
                                model="whisper-1", 
                                file=audio_file
                            )
                        if transcript.text:
                            # 【核心兼容改动】：不再把字回填进框，而是直接将识别出的文本赋给 prompt 变量触发对话！
                            # 这样避开了老版本 st.chat_input 不支持设置初始值的硬伤，体验甚至比手敲更流畅
                            prompt = transcript.text
                    except Exception as e:
                        st.error("识别失败")

        # 4. 核心：文本输入框 (第三列)
        # 彻底去掉破坏老版本兼容性的 value 参数
        user_input = cols[2].chat_input(chat_input_placeholder, key="prompt_input")
        if user_input:
            prompt = user_input
    if prompt:
